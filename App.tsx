...
const MiPymeSeguraLogo = ({ className = "w-10 h-10" }: { className?: string }) => (
  <div className={`${className} bg-[#cc0000] rounded-xl flex items-center justify-center shadow-lg shadow-red-900/10 flex-shrink-0 group-hover:scale-110 transition-transform duration-300 p-1 relative overflow-hidden`}>
    <div className="absolute -top-2 -right-2 w-4 h-4 bg-amber-400 blur-md opacity-30"></div>
...
const Navbar = () => {
  ...
  return (
    <nav className="bg-[#020617]/80 backdrop-blur-xl text-white sticky top-0 z-50 border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <Link to="/admin" className="p-2 text-slate-500 hover:text-[#cc0000] transition-colors ml-4">
...
const Footer = () => {
  ...
  return (
    <footer className="bg-[#020617] text-slate-400 py-24 border-t border-white/5">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-20 pt-8 border-t border-white/5 text-center text-[10px] font-black tracking-[0.4em] uppercase opacity-40 text-slate-500">
        &copy; {new Date().getFullYear()} MI PYME SEGURA. <span className="text-[#cc0000]">STATUS: SECURE</span>
      </div>
    </footer>
  );
};
