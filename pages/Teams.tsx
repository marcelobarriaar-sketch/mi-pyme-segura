...
const EquipmentCard: React.FC<{ 
  ...
  <div className="group relative bg-white/5 rounded-[2.5rem] border border-white/5 overflow-hidden hover:border-amber-400/30 transition-all duration-500 flex flex-col">
    ...
    <div className="p-10 flex-grow flex flex-col">
      ...
      {updates && (
        <div className="mb-6 flex items-start gap-2 bg-amber-400/5 p-3 rounded-xl border border-amber-400/20">
          <History className="w-3 h-3 text-amber-400 mt-0.5" />
          <span className="text-[9px] font-black uppercase tracking-widest text-amber-400 leading-tight">
            NOTAS TÉCNICAS: {updates}
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        {videoUrl && (
          <button 
            ...
            className="w-full py-4 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-2 bg-white/5 text-white border border-white/10 hover:border-amber-400/50 hover:text-amber-400 group/vid"
          >
            Video Demostración <Play className="w-3 h-3 fill-current group-hover/vid:scale-125 transition-transform" />
          </button>
        )}
      </div>
    </div>
    ...
    <div className="animate-in fade-in duration-700 bg-[#020617] py-32">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-24">
          ...
          <h1 className="text-5xl md:text-8xl font-black text-white mb-8 uppercase tracking-tighter italic">NUESTRO <span className="text-amber-400">ARSENAL</span> TÉCNICO</h1>
          <p className="text-xl text-slate-400 max-w-3xl mx-auto font-medium leading-relaxed">
            Catálogo dinámico con respaldo de marcas líderes y soporte local especializado.
          </p>
        </div>
